...
const targetStr = `          <button type="button" class="wa-btn-primary" id="wa-cod-btn" onclick="waPayNow()" style="background:#0f172a; padding:18px; margin-bottom:12px; display:flex; flex-direction:column; align-items:center; gap:2px;">
...
if (code.includes(targetStr)) {
  ...
  fs.writeFileSync('c:\\Users\\HP\\Desktop\\11fit theme\\snippets\\whatsapp-otp-modal.liquid', code);
  console.log('Successfully patched whatsapp-otp-modal.liquid');
} else {
  // Try CRLF normalization just in case
  let nCode = code.replace(/\\r\\n/g, '\\n');
  if (nCode.includes(targetStr.replace(/\\r\\n/g, '\\n'))) {
      nCode = nCode.replace(targetStr.replace(/\\r\\n/g, '\\n'), replaceStr.replace(/\\r\\n/g, '\\n'));
      fs.writeFileSync('c:\\Users\\HP\\Desktop\\11fit theme\\snippets\\whatsapp-otp-modal.liquid', nCode);
      console.log('Successfully patched whatsapp-otp-modal.liquid using LF normalization');
  } else {
      console.log('Target string not found in whatsapp-otp-modal.liquid');
  }
}
