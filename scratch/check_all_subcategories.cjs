...
async function run() {
      ...
      collections(first: 50) {
        edges {
          node {
            ...
            title
            handle
            subcategories: metafield(namespace: "price_editor", key: "subcategories") {
              value
              updatedAt
            }
          }
  ...
  try {
    ...
    console.log(JSON.stringify(res.data.data.collections.edges.filter(e => e.node.subcategories), null, 2));
  } catch (err) {
